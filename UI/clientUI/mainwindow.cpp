...
{
    QMessageBox msgBox(this);
    msgBox.setWindowTitle("Shutdown/Restart");
    msgBox.setText("Bạn muốn tắt hay khởi động lại máy ?");
    QPushButton *shutdownBtn = msgBox.addButton(tr("Shutdown"), QMessageBox::AcceptRole);
    QPushButton *lockBtn = msgBox.addButton(tr("Restart"), QMessageBox::AcceptRole);
    msgBox.addButton(tr("Huỷ"), QMessageBox::RejectRole);
...
void MainWindow::processOneMail()
{
    ...
    // else if (content.startsWith("get_file")) {
    //     QString fname = m_prevSaved;
    //     QFile file(fname);
    //     if (file.open(QIODevice::ReadOnly)) {
    //         QByteArray fileData = file.readAll();
    //         file.close();
